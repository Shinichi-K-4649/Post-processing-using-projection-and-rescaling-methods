function [solinfo,err_flag] = PRwithPrimalModel(A_orig,K,ppinfo)
% Projection and rescaling method with the primal model
...
if isfield(ppinfo.Q.p, "Rescale")
    % Use the scaling information from previous iterations
    Rescale = ppinfo.Q.p.Rescale;
    A = A_orig;
    ...
    u = ppinfo.Q.p.u;
    rescale_check = 1;
    Q_orig_p = ppinfo.Q.p.orig.p;
    Q_orig_d = ppinfo.Q.p.orig.d;
...
while judge == 0
    ...
    if ~isreal(V)
        fprintf("\n numerical error of SVD @ PRojection and Rescaling Method")
        err_flag =1; break
    end
    ...
    [judge,solution,u,Rescale,Q_orig_d,Q_orig_p,A] ...
        = PP_BP_SP(K,Projection,u,xi,Rescale,Q_orig_d,Q_orig_p,A_orig,A);
