function [judge,solution,upper,Rescale,Q_orig_d,Q_orig_p,A] = Alt_PP_BP_SP(K,Projection,upper,eps,Rescale,Q_orig_d,Q_orig_p,A_orig,A)
...
u = u_bar;
k=0;
y = smooth_operator(Projection*u,mu,u_bar,K);
v = y - Projection*y; z = y-v;
...
while true
    ...
    if ~isempty(Index)
        ...
        if ~isempty(index)
            ans_vec(Index(index)) = 1./cut_value(Index(index)); %1/eps;
            tmp = Rescale{1};
        ...
        for i = 1:length(K.s) 
            ...
            if ~isempty(index)
                ...
                ans_vec(Index(index)-start) = 1./sqrt(cut_value(Index(index)));
                Q_output = Q*diag(ans_vec)*Q.';

                ans_vec = zeros(K.s(i),1); ans_vec(Index(index)-start) = 1;
                upper(num_l+i,1) = upper(num_l+i,1) + ...
                    sum(diag(Q_orig_p.right{i+1}*Q*diag(ans_vec)*Q.'*Q_orig_p.left{i+1}.'));

                Q_orig_p.left{i+1} = Q_orig_p.left{i+1}*Q_output.';
                Q_orig_p.right{i+1} = Q_orig_p.right{i+1}*Q_output;
                Tmp = kron(Q_orig_p.left{i+1}, Q_orig_p.right{i+1});
                Rescale{1+i} = Tmp;
                A(:,start_s+1:start_s+K.s(i)^2) = A_orig(:,start_s+1:start_s+K.s(i)^2)*Tmp;


                ans_vec = ones(K.s(i),1);
                ans_vec(Index(index)-start)= sqrt(cut_value(Index(index)-start));
                rescale = Q*diag(ans_vec)*Q.';

                Q_orig_d.left{i+1} = Q_orig_d.left{i+1}*rescale;
                Q_orig_d.right{i+1} = Q_orig_d.right{i+1}*rescale.';
            end
    ...
    theta = 2/(k+3);
    u = (1-theta)*(u+theta*y) + theta^2*smooth_operator(Projection*u,mu,u_bar,K);
    mu = (1-theta)*mu;
    y = (1-theta)*y + theta*smooth_operator(Projection*u,mu,u_bar,K);
    v = y - Projection*y; z = y-v;
    k = k+1;
end
