...
%% Set parameters
para.TimeLimit.P = 1800; para.TimeLimit.D = 1800;
para.Tol = 1e-12;
para.PraTerCon = 30;

%% Start post-processing algorithm
fprintf('\n Starting problem #i= %i  from file   %s \n',num,loadfile)
if mineigK(sol.z,K) > -1e-12
    [bound,TmpSol,PPinfo] = ProposedAlgorithm(A,b,c,K,sol,para);
else
    [bound,TmpSol,PPinfo] = ProposedAlgorithm_type2(A,b,c,K,sol,para);
end
